...
void UGV2TabContainerWidgetBase::ApplyTabEntries(
    ...
    const TMap<FName, UUserWidget*>& InWidgets)
{
    ...
    if (TabContentPanel != nullptr)
    {
        ...
        {
            if (TObjectPtr<UUserWidget> Child = TabScreenWidgets.FindRef(Entry.Key))
            {
    ...
    // Active-tab bookkeeping is technical UI state used to reject interactions from hidden
    // tabs. Preserve the pre-migration behavior by publishing the effective initial/fallback
    // selection through the same narrow sink as a user selection.
    NotifyActiveTabSelection();
}
...
UUserWidget* UGV2TabContainerWidgetBase::GetActiveScreenWidget() const
{
...
UUserWidget* UGV2TabContainerWidgetBase::GetScreenWidgetForTab(FName InTabKey) const
{
...
void UGV2TabContainerWidgetBase::NotifyActiveTabSelection()
{
    const FString ResolvedPath = !ContainerPath.IsEmpty()
        ? ContainerPath
        : (!ConfiguredScreenFieldId.IsNone()
            ? FString::Printf(TEXT("location_content/main/%s"), *ConfiguredScreenFieldId.ToString())
            : FString());
    ...
    if (!ResolvedPath.IsEmpty())
    {
        ...
        {
            Sink->NotifyActiveTab(ResolvedPath, ActiveTabKey.ToString());
        }
    }
}
    ...
    const TArray<GV2PresentationApply::FPreparedTabEntry>& PreparedEntries)
{
    ...
    for (const GV2PresentationApply::FPreparedTabEntry& FlatEntry : PreparedEntries)
    {
        ...
        if (UUserWidget* ScreenWidget = Cast<UUserWidget>(FlatEntry.ScreenWidget.Get()))
        {
