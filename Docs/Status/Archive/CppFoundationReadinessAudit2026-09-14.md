...
Сверх finding blocks раунд оставил одно открытое расхождение — `STATUS-027`. Отдельным finding оно не было: открытый пункт evidence CFC-02 жил условием внутри задачи плана и перенесён в расхождения при архивации.

Тринадцать contract gaps раунда (`STATUS-013…020` и `STATUS-021…025`) переносились в `ImplementationStatus.md` по ходу и удалены оттуда закрывающими задачами; история удалений — в commit. Промежуточный перенос в status означал фиксацию расхождения для планирования, а не исправление кода.
