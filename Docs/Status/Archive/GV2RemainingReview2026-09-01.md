...
|---|---|---|---|
| `REM-01` | P1 | `DeclaredComposite` проверяет child capability только по `kind` (`DoesCapabilityTreeSupportKind`), теряя range/`target_kind`/keyed-identity constraints; подтверждённый пример — `ProgressBar.percent[0..1]` против unbounded declared `Number` | Устранён `GBH-06`…`GBH-08`. Declaration несёт `NumberMin`/`NumberMax`/`IntMin`/`IntMax`/`TargetKind`/`ChildCapabilityName`; `DoesCapabilityTreeSupportKind` заменён `ResolveDelegatedChildCapability` + `IsUiCapabilitySubset` — одна нормализованная subset-проверка, общая с schema↔Widget. Проверено: `GV2.UI.DeclaredComposite.ConstraintsAndSelector` (declaration `[0..100]` на реальном `UGV2ProgressBarWidgetBase[0..1]` отклоняется до любой physical mutation). **Дозакрыто при приёмке плана** (`2cba949`): на момент `source_commit` `IsUiCapabilitySubset` ещё не сравнивал `KeyPropertyName` и `EntryWidgetClass`. Отсрочка первого мотивировалась тем, что `CollectionHost` скрыт и объявить такую constraint некому, — обоснование перестало действовать следующим коммитом, когда `GBH-02B` сделал вид выбираемым. Добавлены сравнения с кодами `KeyPropertyMismatch`/`EntryWidgetClassMismatch` (`GV2.UI.CapabilitySubsetConstraintCoverage`) и обещанный `GBH-08` гейт полноты constraint-полей, которого не существовало: изменение любого поля `FGV2UiPropertyCapability` меняет `sizeof` и ломает сборку, требуя классифицировать поле в таблице рядом с функцией |
