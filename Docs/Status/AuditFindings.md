...
*(Закрыто задачей TSR-08)* Заменяющий smoke-тест `GV2.Runtime.Presentation.RhStartOpensLocationScreen` покрывает присутствие и верификацию полей сцены через `PrepareScreenFields` и зафиксирован в `BASELINE_UE_TESTS` вместо устаревшего диагностического теста.
