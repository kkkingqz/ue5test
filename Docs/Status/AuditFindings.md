...
**Исход:** *(Закрыто задачей SAC-06)* История ограничена 18 записями: UE production-path scenario измеряет 6 terminal operations, а явная retention policy хранит три таких окна. Вытеснение выполняется детерминированно по наименьшему `OperationId`; точное `InProgressOperations` устраняет ложную классификацию out-of-order операций как вытесненных, что подтверждено boundary-контрпримером 1..5.
