...
| `game.commands`, handlers, validators, queue | Реализовано | Синхронный `command_dispatcher` с lookup по ключу, реестры `game.commands.handlers` (CHR-01..07) и `game.commands.validators` (GEW-01) с приоритетом/freeze/canonical order, read-only запуск до открытия mutation window (GEW-02), нормализация refusal envelope `{ code, params }` (GEW-03), валидатор `core:validator.location.travel` (GEW-13), очередь отложенных команд `game.commands.enqueue` ограниченной ёмкости (`MAX_COMMAND_QUEUE_SIZE = 100`) с исполнением в отдельных окнах мутации (план [GameplayEventsAndWorld](../Plans/Archive/GameplayEventsAndWorld/README.md), GEW-12); авторский слой команд с прокси `commands`, помеченными ссылками `__gv2_ref`, отложенной регистрацией, типизированным `fail()` с проверкой `write_revision` и методами `:run()`/`:later()`, semantic action helper `action()` с запретом замыканий (`ActionClosureDisallowed`), `button()` и `show_screen()` с запретом сырых строк (`RawStringDisallowed`), `text()` для канонических `TextSpec` ([ADR-0027](../ADR/0027-designer-lua-authoring-layer.md), план [DesignerAuthoringLayer](../Plans/Archive/DesignerAuthoringLayer/README.md), M2, M5); упрощённая поверхность авторского слоя ([ADR-0028](../ADR/0028-simplified-authoring-surface.md), план [SimplifiedAuthoringSurface](../Plans/Archive/SimplifiedAuthoringSurface/README.md), SAS-01..21) с per-module лексическим `_ENV`, автообнаружением и генерацией манифеста пакета (`generate_manifest.py`), унифицированным доменным API актора (`require_*`, `spend_*`, `travel`, `add_item`), нелокальным выходом `fail()`, единым авторским скриптом `authoring/gameplay.lua`, обобщённой командой `buy(item)` и источником презентации с автоматической инвалидацией после commit вне окна мутации; раздел `Docs/Authoring/` удалён как устаревший и будет воссоздан после стабилизации доменных методов и команд |
