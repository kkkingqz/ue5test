...
| `STATUS-008` | `known_nonconformance` | [ADR-0040 § Decision](../ADR/0040-universal-ui-property-pipeline.md#decision) (пункт 7), [ADR-0040 § Implementation caveat](../ADR/0040-universal-ui-property-pipeline.md#implementation-caveat-ui-schema-authority-status-008) | **Сужено `DCA-19` (2026-09-06):** часть про Decision 6 (владение schema ID по namespace) и фиксированные списки корней (`FGV2UiSchemaCache`, `GV2ScreenFieldMaterializer.cpp`) закрыта — оба теперь читают `GameData/mods.lock.json5` через общий `GV2PackageClosure::DiscoverFromGameData()`, `GameData/sample` (отсутствующий в замыкании) больше не сканируется, а схема с namespace, не совпадающим с пакетом, под которым она физически найдена, не регистрируется. Остаётся только Decision 7 (политика отказа зависит от владельца схемы): владелец схемы определяется в `Source/GV2/Private/UI/GV2UiCapability.cpp:12` (`IsModNamespace`) закрытой тройкой литералов `core`/`textsystem`/`rh` — дословной копией содержимого `mods.lock.json5`, — и результат нигде не потребляется: `Diag.bFatal = !bIsMod` не читает ни один production-путь, все три вызывающих (`GV2UiMutationPlan.cpp:147`, `GV2PropertyConsumers.cpp:1291`, `GV2UiPropertyHost.h:48`) ветвятся по возвращаемому значению `IsModNamespace`, а `bSuccess = false` выставляется для мода и первопартийной схемы одинаково; на `bFatal` ссылаются только два утверждения в `GV2UiPropertyHostTests.cpp:93,184`. Decision 7 тем самым не «не наблюдалась», а вычисляется и отбрасывается. **Условие закрытия:** `DCA-20` — Decision 7 действует и наблюдаема в поведении сессии, либо `bFatal`/`IsModNamespace` физически удалены вместе с ADR-0040 Decision 7. | `Source/GV2/Private/UI/GV2UiCapability.cpp:12` (`IsModNamespace`), `:421` и далее (`Diag.bFatal`); `Source/GV2/Private/UI/GV2UiMutationPlan.cpp:147`, `Source/GV2/Private/UI/GV2PropertyConsumers.cpp:1291`, `Source/GV2/Public/UI/GV2UiPropertyHost.h:48`; `Source/GV2/Private/Tests/GV2UiPropertyHostTests.cpp:93,184`; `GameData/mods.lock.json5`. |
