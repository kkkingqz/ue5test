...
| `STATUS-012` | `known_nonconformance` | [ADR-0042 § INV-P5](../ADR/0042-presentation-authority-and-publication.md) (семантика решается до подготовленной транзакции) | `FGV2ImageResourcePropertyConsumer::Prepare` резолвит ресурс изображения через каталог и проверяет по нему режим отрисовки и соотношение сторон, но сохраняет только `PreparedResourceId`, отбрасывая разрешённый результат. `Commit` резолвит тот же id заново — напрямую (`FGV2ImagePresentation::ResolveAndApply` для голого `UImage`) и через хосты (`UGV2ImageWidgetBase::ApplyImageResource`, `UGV2PortraitWidgetBase::ApplyPortrait`), — то есть авторитет опрашивается на фазе применения, и применяется значение, выведенное заново, а не то, которое подготовка одобрила. Сегодня безвредно: каталог пиннингован на сессию и между фазами не меняется. Найдено гейтом `PAH-08` через транзитивную достижимость `Commit -> ResolveAndApply`, а не прямым сканом тел `Commit*` — именно та косвенность, ради которой гейт двухчастный. **Условие закрытия:** подготовленное значение несёт разрешённый ресурс, и применение его не перерезолвивает; маркер `phase=commit_resolve_deferred=STATUS-012` в `GV2ImagePresentation.cpp` снимается. | `Source/GV2/Private/UI/GV2ImagePresentation.cpp`, `Source/GV2/Private/UI/GV2PropertyConsumers.cpp` (`Prepare` :325-360, `Commit` :363-395), `Tools/Testing/validate_presentation_authority_phase.py`. |
