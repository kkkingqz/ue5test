...
| `game.instances`, Runtime Instance Registry | Реализовано | Реализован реестр `game.instances.actors` (`get`/`exists`/`create`/`remove`/`ids`/`player`), disposable `ActorWrapper` с динамическим `discriminator`, фабриками-декораторами `register_type`, freeze на фазе `register` и типизированным отказом `ActorTypeNotRegistered`; singleton `game.instances.world()` возвращает disposable wrapper над `state.world`, не кэшируется, отклоняется валидатором state при попытке сохранения; схемы предметов и локаций перенесены в `rh` (планы [GameplayEventsAndWorld](../Plans/Archive/GameplayEventsAndWorld/README.md), [CoreBoundaryMigration](../Plans/CoreBoundaryMigration/README.md)) |
