...
| `STATUS-018` | `partial` | [ADR-0043 § D1](../ADR/0043-presentation-apply-boundary.md) (Prepare разрешает authority только через `FGV2PresentationPrepareContext`, не статический accessor) | `FGV2RichTextSpansPropertyConsumer::Prepare()`'s проверка доступности hover-popover-рендерера (`bHasHover` branch) с `PSC-10A` тоже читает Theme через `PrepareContext->GetTheme()` вместо `GetConfiguredTheme()`, когда контекст задан — тот же паттерн, что уже red-on-revert-доказан для `FGV2TabContainerTabsPropertyConsumer`/`FGV2ImageResourcePropertyConsumer`. Но ни один production, ни тестовый путь сегодня не исполняет эту ветку с непустым `PrepareContext` одновременно: ни один `GameData`-контент не объявляет span `hover`, а единственный существующий тест на эту проверку (`GV2RuntimeSubsystemTests.cpp`, `REV3-09`) строит consumer напрямую, никогда не вызывая `SetPrepareContext()` — значит всегда идёт по неизменённой legacy-ветке. Red-on-revert (временный forced-`nullptr` Theme при `PrepareContext != nullptr`) дал 0 упавших тестов из 128 — подтверждает, что PrepareContext-ветка сейчас не нагружена ничем. Наблюдаемого нарушения правила нет (код корректен, ADR-0043 D1 не нарушен), но заявление «эта ветка проверена red-on-revert'ом» было бы ложным без этой строки. **Условие закрытия:** либо `GameData` получает контент со span hover и связанный production-путь его реально нагружает, либо `REV3-09` (или новый тест) строит consumer с реальным `SetPrepareContext(...)` через настоящий `FGV2SessionContentSnapshot`/`FGV2SessionContentCandidate::Build()`. | `Source/GV2/Private/UI/GV2PropertyConsumers.cpp` (`FGV2RichTextSpansPropertyConsumer::Prepare()`, `bHasHover` branch); `Source/GV2/Private/Tests/GV2RuntimeSubsystemTests.cpp` (`REV3-09`, не вызывает `SetPrepareContext`); commit `6e462c3` (`PSC-10A (3/N)`, red-on-revert результат записан в теле коммита). |
