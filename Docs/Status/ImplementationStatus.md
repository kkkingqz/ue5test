...
| `game.instances`, Runtime Instance Registry | Реализовано | Реализован реестр `game.instances.actors` (`get`/`exists`/`create`/`remove`/`ids`/`player`), disposable `ActorWrapper` с динамическим `discriminator`, фабриками-декораторами `register_type`, freeze на фазе `register` и типизированным отказом `ActorTypeNotRegistered`; singleton `game.instances.world()` возвращает disposable wrapper над `state.world`, не кэшируется, отклоняется валидатором state при попытке сохранения; схемы предметов и локаций перенесены в `rh` (планы [GameplayEventsAndWorld](../Plans/Archive/GameplayEventsAndWorld/README.md), [CoreBoundaryMigration](../Plans/Archive/CoreBoundaryMigration/README.md)); авторский слой свойств (`core:module.authoring.properties`, `DLA-10..13`, [ADR-0027](../ADR/0027-designer-lua-authoring-layer.md)), атрибуты схем `storage`/`write_policy`/`operations`, схемная валидация записей обёртки до мутации, проверка доменных операций managed-полей на freeze, разрешение `ref_definition` и `ref_instance`, реестро-ориентированная ссылочная целостность при удалении сущностей |
