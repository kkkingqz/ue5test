...
| `gv2-content` CLI и Live Loop | Реализовано | `validate` (включая `--watch`), `inspect`, `describe`, `new`, `refs`, `rename` (с проверкой `package_frozen`), `set` (точечная правка поля по JSON Pointer без разрушения комментариев и форматирования), `delete` (удаление записи с проверкой входящих ссылок), `index`, `hash`, `coverage` (отчёт о полноте перевода PO-каталогов), стабильные exit codes (планы [ContentAuthoringTools](../Plans/Archive/ContentAuthoringTools/README.md), [ContentCliModularization](../Plans/Archive/ContentCliModularization/README.md) и M2 плана [ContentEditorPrerequisites](../Plans/ContentEditorPrerequisites/README.md)); модульная архитектура CLI с разделением на изолированные команды в `Tools/Content/Source/Commands/` и сервисы поддержки в `Tools/Content/Source/Support/` (план [ContentCliModularization](../Plans/Archive/ContentCliModularization/README.md), M1–M4); интеграция с редакторами (`.vscode/tasks.json` с problemMatcher, `.vscode/settings.json`, `Tools/Editor/generate_vscode_snippets.py`, `Tools/Editor/README.md`) |
