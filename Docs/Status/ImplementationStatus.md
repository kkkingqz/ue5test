...
| Package discovery and ordering | Реализовано | Discovery набора пакетов по списку корней или контейнерному каталогу (`IsContainerDirectory`, `DiscoverPackagesFromContainer`), топологическая сортировка зависимостей, назначение load_index, проверка зависимостей, циклов и load_after, генерация и сверка mods.lock.json5, FMultiPackageSourceProvider, детерминизм и сейвы (план [PackageSupport](../Plans/Archive/PackageSupport/README.md), M1–M5; M1 плана [TextSystemLayer](../Plans/TextSystemLayer/PackageSet.md)); базовый набор поставки разделён на движок `core`, переиспользуемый текстовый слой `textsystem` ([ADR-0030](../ADR/0030-textsystem-layer-and-data-driven-package-set.md)) и игру `rh`, демо-контент изолирован в `sample`, схемы `item`, `location` и расширения актора вынесены в `rh`, динамический реестр ссылочных полей состояния в `state_validator.lua`, обратные ссылки и игровые сущности в ядре блокируются CI-гейтами `core_decoupling_gate_contract` и `core_boundary_gate_contract` (планы [RhGamePackage](../Plans/Archive/RhGamePackage/README.md) и [CoreBoundaryMigration](../Plans/Archive/CoreBoundaryMigration/README.md)) |
...
| `game.instances`, Runtime Instance Registry | Реализовано | Реализован реестр `game.instances.actors` (`get`/`exists`/`create`/`remove`/`ids`/`player`), disposable `ActorWrapper` с динамическим `discriminator`, фабриками-декораторами `register_type`, freeze на фазе `register` и типизированным отказом `ActorTypeNotRegistered`; singleton `game.instances.world()` возвращает disposable wrapper над `state.world`, `world.current_location` является read-only аксессором к игроку, не кэшируется, отклоняется валидатором state при попытке сохранения; схемы предметов и локаций перенесены в `rh` (планы [GameplayEventsAndWorld](../Plans/Archive/GameplayEventsAndWorld/README.md), [CoreBoundaryMigration](../Plans/Archive/CoreBoundaryMigration/README.md)); авторский слой свойств (`core:module.authoring.properties`, `DLA-10..16`, [ADR-0027](../ADR/0027-designer-lua-authoring-layer.md)), атрибуты схем `storage`/`write_policy`/`operations`, схемная валидация записей обёртки до мутации, проверка доменных операций managed-полей на freeze, разрешение `ref_definition` и `ref_instance`, реестро-ориентированная ссылочная целостность при удалении сущностей, sparse-материализация полей определений с fallback на schema default, явный `:reset(field)` и регистрация декораторов определений `register_definition_type` |
...
| `game.commands`, handlers, validators, queue | Реализовано | Синхронный `command_dispatcher` с lookup по ключу, реестры `game.commands.handlers` (CHR-01..07) и `game.commands.validators` (GEW-01) с приоритетом/freeze/canonical order, read-only запуск до открытия mutation window (GEW-02), нормализация refusal envelope `{ code, params }` (GEW-03), очередь отложенных команд `game.commands.enqueue` ограниченной ёмкости (`MAX_COMMAND_QUEUE_SIZE = 100`) с исполнением в отдельных окнах мутации (план [GameplayEventsAndWorld](../Plans/Archive/GameplayEventsAndWorld/README.md), GEW-12); команда перемещения `rh:command.travel` объявлена в игре поверх доменных операций `textsystem` (план [TextSystemLayer](../Plans/TextSystemLayer/README.md), M2); авторский слой команд с прокси `commands`, помеченными ссылками `__gv2_ref`, отложенной регистрацией, типизированным `fail()` с проверкой `write_revision` и методами `:run()`/`:later()`, semantic action helper `action()` с запретом замыканий (`ActionClosureDisallowed`), `button()` и `show_screen()` с запретом сырых строк (`RawStringDisallowed`), `text()` для канонических `TextSpec` ([ADR-0027](../ADR/0027-designer-lua-authoring-layer.md), план [DesignerAuthoringLayer](../Plans/Archive/DesignerAuthoringLayer/README.md), M2, M5); упрощённая поверхность авторского слоя ([ADR-0028](../ADR/0028-simplified-authoring-surface.md), план [SimplifiedAuthoringSurface](../Plans/Archive/SimplifiedAuthoringSurface/README.md), SAS-01..21) с per-module лексическим `_ENV`, автообнаружением и генерацией манифеста пакета (`generate_manifest.py`), унифицированным доменным API актора (`require_*`, `spend_*`, `move_to`/`travel`, `add_item`), нелокальным выходом `fail()`, единым авторским скриптом `authoring/gameplay.lua`, обобщённой командой `buy(item)` и источником презентации с автоматической инвалидацией после commit вне окна мутации; раздел `Docs/Authoring/` удалён как устаревший и будет воссоздан после стабилизации доменных методов и команд |
...
Играбельный цикл существует: план [TestGameplaySlice](../Plans/Archive/TestGameplaySlice/README.md) собрал в пакете `rh` три локации с картой `market↔tavern↔gate`, перемещение за выносливость, покупки и заработок. План [TextSystemLayer](../Plans/TextSystemLayer/README.md) выделил текстовый движок `textsystem` (акторы, локации, презентация), перенёс сборку экрана локации в `textsystem` на основе семантических действий (`game.actions`), а экранные меню и действия выражаются декларативно через определения контента.
