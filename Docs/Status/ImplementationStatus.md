...
| `STATUS-005` | `partial` | [Screen Templates § Apply lifecycle](../UI/ScreenTemplates.md) шаг 4, [§ LocationScene Field Contract](../UI/ScreenTemplates.md) | Contract требует отклонять `unknown payload field` и объявляет любое несоответствие формы поля типизированным отказом применения. Реализована только проверка уровня `field_id`: неизвестный идентификатор поля отклоняется, а неизвестный **ключ внутри значения поля** молча игнорируется. Каждый `Build*` читает известные ему ключи и не сверяет набор ключей со схемой, поэтому опечатка автора контента не наблюдаема ничем. | `Source/GV2/Private/UI/GV2ScreenWidgetBase.cpp` (проверка `unknown field` работает по `ConsumedFieldIds`), `Source/GV2/Private/Application/GV2ScreenFieldAdapterRegistry.cpp` (`BuildLocationScene` принимает объект персонажа с любыми лишними ключами). Наблюдаемое следствие — `item_icon_resource_ids`/`effect_icon_resource_ids` пересекают границу и выбрасываются без диагностики. |
| `STATUS-006` | `known_nonconformance` | [UI Document and Reconciliation § Атомарность публикации](../UI/UIDocumentAndReconciliation.md) | Contract требует, чтобы отказ кандидата не мутировал активный набор. `FGV2KeyedCollection::Reconcile` после фазы валидации вызывает `ApplyItem` прямо на переиспользуемых живых виджетах и выходит по первому отказу, оставляя уже применённые элементы в новом состоянии. Doc-комментарий самого класса утверждает обратное дословно: «fails without mutating the live UI if keys are invalid, widget creation fails, **or item apply fails**». | `Source/GV2/Public/UI/GV2KeyedCollection.h`, `Source/GV2/Public/UI/GV2ListViewWidgetBase.h:15-16`; `GV2.Runtime.UI.CompositeRollbackContract` после `ReconcileEntries == false` проверяет только `GetChildrenCount()` и не смотрит состояние переиспользованных виджетов. |
| `STATUS-007` | `known_nonconformance` | [Screen Templates § Apply lifecycle](../UI/ScreenTemplates.md) шаг 8 | Contract разрешает считать применение успешным только после полного success. Четыре пути композитов локации при неразрешённом классе дочернего виджета пропускают весь блок reconcile и доходят до `Applied = Candidate; return true`: непустая коллекция сообщает успех, не отрисовав ни одного элемента. Эталон обратного поведения уже существует в этой же кодовой базе. | Пропуск: `Source/GV2/Private/UI/GV2LocationCompositeWidgetBases.cpp:203` (meters), `:226` (items), `:249` (effects), `:417` (characters). Отказ: `UGV2ButtonListWidgetBase::CanApplyButtonModels`, `UGV2DropdownSelectWidgetBase::CanApplyDropdownModel` возвращают `false` на том же условии. |
