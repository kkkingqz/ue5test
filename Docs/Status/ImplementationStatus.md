...
| `game.commands`, handlers, validators, queue | Реализовано | Синхронный `command_dispatcher` с lookup по ключу, реестры `game.commands.handlers` (CHR-01..07) и `game.commands.validators` (GEW-01) с приоритетом/freeze/canonical order, read-only запуск до открытия mutation window (GEW-02), нормализация refusal envelope `{ code, params }` (GEW-03), валидатор `core:validator.location.travel` (GEW-13), очередь отложенных команд `game.commands.enqueue` ограниченной ёмкости (`MAX_COMMAND_QUEUE_SIZE = 100`) с исполнением в отдельных окнах мутации (план [GameplayEventsAndWorld](../Plans/Archive/GameplayEventsAndWorld/README.md), GEW-12); авторский слой команд с прокси `M.commands`, помеченными ссылками `__gv2_ref`, отложенной регистрацией, типизированным `fail()` с проверкой `write_revision` и методами `:run()`/`:later()`, semantic action helper `action()` с запретом замыканий (`ActionClosureDisallowed`), `button()` и `show_screen()` с запретом сырых строк (`RawStringDisallowed`), `text()` для канонических `TextSpec` ([ADR-0027](../ADR/0027-designer-lua-authoring-layer.md), план [DesignerAuthoringLayer](../Plans/DesignerAuthoringLayer/README.md), M2, M5) |
| `game.events` (EventBus), post-commit facts | Реализовано | Конверт событий (`core:module.runtime.event_envelope`, GEW-06), post-commit доставка и изоляция контекста (GEW-07), приоритет подписчиков и детерминированная breadth-first доставка (GEW-08), отслеживание runtime phases и pump limit breach (`core:module.runtime.event_bus`, GEW-09); реестр подписок `game.events.subscribers` (`core:module.runtime.subscriber_registry`) с freeze на фазе `register` (GEW-10); закрытое mutation window обработчиков и сохранение committed state при ошибке (GEW-11); авторский слой событий `emit`/`on` с разворачиванием обёрток в `__gv2_ref` и регидрацией свежих обёрток, читающих текущее committed состояние ([ADR-0027](../ADR/0027-designer-lua-authoring-layer.md), M5); инструмент `Tools/Content/collect_texts.py` для сбора литералов `text()`/`fail()` и синхронизации PO-каталогов; этапы M3, M4 и M5 закрыты |
