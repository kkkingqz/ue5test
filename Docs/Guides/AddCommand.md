...
**1. Выбрать `command_id`.** Kind `command`, например `core:command.shop.buy`. Имя описывает действие, а не запись в поле: `set_gold` недопустимо как публичная команда.

**2. Написать валидатор, если действие может быть недопустимо.** Валидатор только читает состояние и репозиторий, ничего не меняет и возвращает либо разрешение, либо типизированный отказ с кодом kind `error`. Регистрируется на фазе `register`.

**3. Написать обработчик.** Он остаётся тонким: связывает команду с сервисом или доменным методом и не содержит правил.

```lua
if request.command_id == "core:command.shop.buy" then
    return game.services.trade.buy(request.args)
end
```

**4. Разместить логику.** Операция над одной сущностью — метод обёртки (`actor:add_gold`). Сценарий над несколькими — Gameplay Service.

**5. Добавить спеку.** Как минимум: успешный путь, отказ валидатора (состояние не изменилось), отсутствие изменения хэша состояния при отказе. См. [AddLuaSpec](AddLuaSpec.md).
...
**Синхронный вызов команды из команды.** Запрещён; отложенная команда ставится в очередь и выполнится в собственном окне.
