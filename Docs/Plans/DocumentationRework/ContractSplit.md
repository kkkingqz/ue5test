...
## Карта выполненной миграции

| Исходный раздел | Итоговый владелец |
|---|---|
| Invariants, Standard libraries, Module loader, Source layout, Global environment | `LuaRuntimeContract.md`; authoring `_ENV` → `AuthoringSurfaceContract.md` |
| Stable `game` façade | `RuntimeFacadeAndRegistries.md`; repository query semantics → `GameDataRepositoryContract.md` |
| Command handlers/validators/queue, EventBus | `CommandsAndEvents.md`; общий freeze → `RuntimeFacadeAndRegistries.md` |
| Value model, protected execution, determinism, technical ingress, GC/diagnostics | `LuaRuntimeContract.md` |
| Canonical state, runtime instances, entity effective methods | `CanonicalStateAndSave.md` |
| Gameplay Services | runtime semantics → `CommandsAndEvents.md`; declaration syntax → `AuthoringSurfaceContract.md` |
| Semantic Actions | binding semantics → `UI/SemanticInput.md`; declaration syntax → `AuthoringSurfaceContract.md` |
| Designer authoring layer | `AuthoringSurfaceContract.md`; Screen/Presentation result semantics остаются в `UI/` |
| Conformance | разнесён по verification-разделам каждого owner contract |

## Задачи

- [x] **DOC-07 — Выделить контракт авторского слоя**
...
- [x] **DOC-08 — Выделить контракт фасада и реестров**
  - Зависимости: DOC-07.
...
- [x] **DOC-09 — Целостность после разделения**
  - Зависимости: DOC-08.
...
- [x] `LuaRuntimeContract.md` не содержит разделов про фасад, реестры и авторский слой.
- [x] У каждого правила ровно один владелец.
- [x] Общий facade contract маршрутизирует к subsystem contracts и не повторяет их семантику.
- [x] UI contract сохранил ownership Screen Document и Presentation semantics.
- [x] Ссылки ведут к владельцу правила, а не к прежнему пути.
- [x] Карта зависимостей и реестр инвариантов обновлены.
