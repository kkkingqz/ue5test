...
- [x] **CED-13 — Модель изменённых полей и сохранение**
  ...
  - Done: редактор отслеживает исходное значение, текущее значение и признак изменения; сохранение собирает набор изменённых полей и применяет его **одной** операцией библиотеки; несколько независимых записей не выполняются ни при каких условиях; authoritative-валидация выполняется над candidate до замены файла, после успеха каноническое представление перечитывается, ссылки обновляются, признаки изменения снимаются; автосохранение не вводится, доступны явные сохранение и перечитывание.
