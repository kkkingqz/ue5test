...
- [ ] **CVA-01 — Создать ADR по авторским валидаторам**
  ...
  - Evidence: `Docs/ADR/0034-*.md`, `Docs/ADR/README.md`. Номер 0033 закреплён за UIC-01 плана [UiComposition](../UiComposition/README.md); если тот ADR ещё не принят к моменту выполнения, номера меняются местами, а ссылки обновляются в том же change set.
...
- [ ] В `Scripts/` не осталось имён игровых параметров.
- [ ] Позиционная, пустая и табличная формы декодируются одной функцией.
- [ ] Переход между локациями и покупка работают как прежде.
