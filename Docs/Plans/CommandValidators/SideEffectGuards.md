...
- [ ] Каждая из четырёх точек отклоняется из валидатора с `AuthoringValidatorSideEffectDisallowed`.
- [ ] Разрешённые операции чтения продолжают работать.
- [ ] Отложенный вызов проходит цепочку валидаторов.
- [ ] Перечень точек записан в contract; добавление нового helper с эффектом обязано его расширять.
