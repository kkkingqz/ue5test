...
- [ ] **M2 — [Authoring Prototypes](AuthoringPrototypes.md)**: инжекция контролируемых прокси-прототипов (`Actor`, `Location`, `Quest`, `Item`) в `_ENV` authoring-скриптов, сбор деклараций через `__newindex`, контекст пакета для `fail()`, интеграция с валидацией managed properties (DLA-12).
