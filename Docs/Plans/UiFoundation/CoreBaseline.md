...
- [ ] **UIF-13 — Обобщённый список**
  ...
  - Done: реализован обобщённый список с политикой раскладки (вертикальная, горизонтальная); элемент списка может быть композитом любого слоя; идентичность детей подчиняется правилу ключей M1 без исключений; `ButtonList` выражен через него либо сохранён как частный случай с общей реализацией повторения; двумерное размещение в этот этап не входит.
...
- [ ] Из базового набора собирается экран со списком композитов внутри области прокрутки.
- [ ] `Image`, `Portrait` и `Progress` управляются данными.
- [ ] Список подчиняется правилу ключей.
- [ ] Ни один элемент не обходит конвейеры текста и графики.
- [ ] Элементов без потребителя не добавлено.
