...
- [x] Документ без `key` у повторяемого элемента и с дубликатом ключа отклоняется до apply; изменение порядка не пересоздаёт виджеты.
- [x] Разрешение авторинга растра и единицы раскладки заданы как два разных значения в одном месте.
- [x] Раскладка распределяет фактический viewport; равномерное масштабирование кадра нигде не используется.
- [x] Каждый визуальный примитив объявляет политику масштабирования; отсутствие объявления и несовместимость с режимом ресурса отклоняются.
- [x] Текст масштабируется отдельной кривой и не опускается ниже минимального читаемого размера.
- [x] На 1280×720 все обязательные элементы управления доступны, критический текст не потерян, недостижимого контента нет.
- [x] UI проходит матрицу из шести разрешений, включая две пропорции 21:9.
- [x] Одинаковый семантический стиль даёт одинаковый физический размер во всех виджетах; одинаковый `resource_id` разрешается одинаково.
- [x] `Image`, `Portrait` и `Progress` управляются данными через контракты Screen Field.
- [x] Обобщённый список заменяет частный случай `ButtonList` и подчиняется правилу ключей.
- [x] Одновременно существуют маршрут, оверлей и модалка; модалка блокирует нижние слои.
- [x] Ревизия, меняющая одно поле, не пересоздаёт остальные экземпляры и элементы.
- [x] Переключение вкладки не порождает команду, не меняет ревизию документа и не попадает в сейв; handle неактивной вкладки отклоняется.
- [x] Класс экрана из namespace пакета вне каталога своего слоя — ошибка валидации реестра.
- [x] Конфигурация без `textsystem` отрисовывает `core:screen.error`, `core:screen.loading` и `core:screen.recovery`.
