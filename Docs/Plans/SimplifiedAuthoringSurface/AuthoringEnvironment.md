...
- [ ] Designer-файл не содержит `M`, `return M` и вызова создания контекста.
- [ ] Глобальная переменная в authoring-скрипте отвергается.
- [ ] Команда без явного результата считается успешной.
- [ ] Таблица с `ok = false`, возвращённая не через `fail()`, отказом не является.
- [ ] Спека проверяет правило через диспетчеризацию.
