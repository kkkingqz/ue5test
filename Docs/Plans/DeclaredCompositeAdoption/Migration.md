...
- [x] **DCA-08 — Специальный путь под конкретный виджет удалён и не может вернуться**
  ...
  - Evidence: `Source/GV2/Private/UI/GV2UiMutationPlan.cpp`, `Source/GV2/Private/Tests/GV2PropertyConsumersTests.cpp`.
  - **Реализация (2026-09-03):** блок к моменту старта задачи уже не содержал ни одного `Cast` по конкретному классу — `DCA-05…07` удалили все три ветки вместе с классами. Работа `DCA-08` сводилась к тому, чтобы это состояние стало **проверяемым**, а не просто наблюдаемым фактом: добавлен раздел «4b. DCA-08» в `FGV2PropertyConsumersTest` (`GV2.UI.StandardPropertyConsumers`, `GV2PropertyConsumersTests.cpp`), реализующий гейт по образцу `GBH-05`.

    Гейт рефлективно перечисляет все native-классы, реализующие `IGV2ScreenFieldHost` или `IGV2UiPropertyHost` (`TObjectIterator<UClass>` + `ImplementsInterface`) — множество определяется интерфейсом, а не жёстким списком имён, поэтому новый composite-хост попадает под правило автоматически. Для каждого имени класса проверяется отсутствие подстроки `Cast<ИмяКласса>` в исходном тексте `GV2UiMutationPlan.cpp`. Отдельно скан по всему `Source/GV2` (рекурсивно, с исключением своего файла — тот же приём, что и `GBH-05`) подтверждает, что уникальная сигнатура reflection-fallback'а разрешения цели (`FindFProperty<FObjectPropertyBase>(HostWidget->GetClass(), Cap.TargetName)`) существует ровно в одном файле — дублирование этой логики в другой файл гейт обнаружит как нарушение условия уникальности.

    Отрицательный самотест встроен в сам гейт: синтетическая строка с `Cast<UGV2ButtonWidgetBase>` пропускается через ту же функцию-проверку, что и реальный файл, и обязана быть обнаружена — это доказывает, что проверка работает, а не проходит вхолостую за неимением находок.

    Red→green продемонстрирован дважды на реальных файлах (не только на синтетической строке из самотеста): временная вставка `// Cast<UGV2ButtonWidgetBase>` в `GV2UiMutationPlan.cpp` уронила `GV2.UI.StandardPropertyConsumers` с `found: UGV2ButtonWidgetBase`; отдельно временное дублирование маркера reflection-fallback'а (в виде комментария) в `GV2UiCapabilityObservability.cpp` уронило тот же тест с «exists in exactly one file… but it was 2» — оба условия гейта подтверждены как реально работающие, а не только теоретически описанные. Оба временных изменения отменены; рабочее дерево после демонстрации идентично состоянию до неё.

    Верификация: portable ctest 76/76; полный `GV2.*` UE Automation (live editor) 99/99 без единого фейла.

## Проверка milestone

- [x] Ни одного C++-класса виджета, существующего только ради композиции, в исходниках не осталось.
- [x] Разрешение цели не упоминает ни одного конкретного класса виджета; для трёх удалённых классов это гарантирует компилятор, для будущих — гейт с отрицательным самотестом.
- [x] Экран локации работает без изменения поведения, проверено от Lua до состояния виджетов.
- [x] Sweep наблюдаемости покрывает все три новых объявляемых композита.

**Закрытие проверки (2026-09-04).** Первые три пункта подтверждены по коду: `GV2LocationCompositeWidgetBases.h/.cpp` отсутствуют, а в `Public/UI/` не осталось ни одного класса, существующего ради композиции — только примитивы и инфраструктура; гейт `DCA-08` выводит множество через `TObjectIterator<UClass>` + `ImplementsInterface`, имеет отрицательный самотест и проверяет единственность места разрешения цели; sweep берёт множество из `CollectProductionUiPropertyHostImplementations()`.

Четвёртый пункт покрыт не был. `GV2.Runtime.Presentation.RhStartOpensLocationScreen` шёл от Lua до **структуры** — находил три композита по `HostIdentity`, читал число элементов репитеров, подавал реальное взаимодействие, — но ни одного утверждения о том, что опубликованное Lua **значение** дошло до листа, в нём не было. Между тем именно это и есть риск миграции: структура остаётся верной, счётчики сходятся, а свойство молча перестаёт приходить.

Добавлены два независимых утверждения, оба с перечислителем:

1. **Учётная сторона.** Для каждого композита множество берётся пересечением двух независимо произведённых сторон — что связывает объявление Designer и что объявляет зафиксированная схема. Схемно-обязательное поле обязано иметь зафиксированное значение; кроме того, каждый композит обязан получить хотя бы одно объявленное значение. Второе утверждение существует потому, что первого недостаточно: `textsystem:schema.ui_field.location_scene.v1` объявляет **все** поля верхнего уровня `required: false`, поэтому ревизия, в которой не пришло ничего, удовлетворяла бы каждую обязательную проверку этой схемы. Записано отдельно как [STATUS-011](../../Status/ImplementationStatus.md).
2. **Физическая сторона.** Лист резолвится через само объявление (`ChildWidgetName` записи с нужным `PropertyName`), а не по зашитому имени виджета, и его отрисованный текст обязан быть непустым.

Red→green показан на стороне данных, а не кода: удаление `context_text` из полезной нагрузки `scene` в `location_presenter.lua` роняет утверждение о листе; опустошение всей полезной нагрузки `scene` роняет счётчик пришедших значений (`scene=0, player_status=5, commands=1`). Возврат — снова зелёный.
