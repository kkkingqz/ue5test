...
- [ ] **DCA-01 — Объявление выражает необязательное свойство**
  ...
  - Done: запись объявления получает признак необязательности, редактируемый в Designer; необязательное свойство при непривязанном ребёнке **не объявляется** и не отказывает; обязательное при непривязанном ребёнке отказывает до `Ready` с прежней типизированной диагностикой — оба случая покрыты, и положительный, и отрицательный; поведение схемы не меняется: отсутствующее необязательное свойство остаётся отсутствующим полем, а не появляется пустым; контракт описывает, чем необязательность объявления отличается от необязательности поля схемы.
...
- [ ] **DCA-02 — Хосты коллекций существуют в дереве виджетов**
  ...
  - Done: в `WBP_PlayerStatusPanel`, `WBP_SceneView` и `WBP_CommandPanel` каждая коллекция имеет привязанный `WBP_ListView` (или эквивалентный виджет-репитер), присутствующий в `WidgetTree`; голые панели как хосты коллекций не используются; ассеты изменены через `unreal-mcp` с compile и save; automation зелёная **до** удаления мостика — на этом шаге поведение не меняется, меняется только форма ассета.
...
- [ ] **DCA-03 — Класс элемента задаётся явно**
  ...
  - Done: класс элемента каждой коллекции задан явно там, где коллекция объявляется; проверено, что ни одна из четырёх виртуальных функций никем не переопределена, и они удалены вместе с зашитыми путями; отсутствие класса элемента у непустой коллекции остаётся отказом, а не подстановкой умолчания; тест краснеет при возврате отката по фиксированному пути.
...
- [ ] **DCA-04 — Мёртвая поверхность трёх композитов удалена**
  ...
  - Done: все три сущности физически удалены из заголовка и реализации, а не помечены; проверено, что ни один Blueprint на них не ссылается — поиск по всем `.uasset`, а не по трём известным; ассеты, если ссылались, приведены через `unreal-mcp`; гейт запрета legacy-поверхности расширен на эти имена, чтобы они не вернулись.
