...
- [ ] **SVC-05 — Матрица разрешений измеряет выделенную геометрию**
  ...
  - Done: тест выполняет раскладку и читает **выделенную** геометрию каждого блока; сравнение ширины сцены на 16:9 и 21:9 различает значения и падает, если дополнительная ширина уходит не сцене; проверка 720p опирается на фактические границы элементов, а не на число записей репитера; при этом проверяется, что все команды помещаются в выделенную область; порог верхней панели содержателен на всех шести разрешениях.
...
- [ ] **SVC-06 — Снять вакуумный тест перехода**
  ...
  - Done: тест либо выполняет переход и проверяет то, что заявляет, либо удалён как дублирующий `GV2.Runtime.UI.LocationScreenTransitionContract`, который переход выполняет; выбор обоснован в change set; в наборе не остаётся теста, чьё имя обещает больше, чем делает тело.
  - Evidence: `Source/GV2/Private/Tests/GV2RuntimeSubsystemTests.cpp`.
...
- [ ] Матрица различает 16:9 и 21:9 по фактической геометрии.
- [ ] На 720p проверяется размещение команд, а не их количество.
- [ ] Вакуумных тестов в наборе не осталось.
- [ ] Handle неактивной вкладки отклоняется.
