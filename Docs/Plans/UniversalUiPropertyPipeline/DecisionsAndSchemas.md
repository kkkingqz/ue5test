...
- [x] **UPP-03 — `schema_ref` и обнаружение циклов**
  - Зависимости: UPP-02.
  ...
  - Evidence: `Source/GV2ContentCore/Public/GV2ContentCore/UiSchema.h`, `Source/GV2ContentCore/Private/UiSchema.cpp` — интерфейс `IUiSchemaResolver`, реализация `FInMemoryUiSchemaResolver`, проверка границ владения `IsAllowedSchemaRefNamespace` и разрешение `schema_ref` в `CompileUiFieldSpec` с обнаружением циклов по `ActiveResolutionChain`. Тесты `RunUiSchemaConformance` (`Source/GV2ContentCore/Private/UiSchemaConformance.cpp`) покрывают 10 новых сценариев (inlining без runtime kind, распространение nullability, невалидный ID, неразрешённая схема, нарушение namespace границы, прямой цикл длины 1, косвенный цикл длины 2, косвенный цикл длины 3, замкнутость спецификации, отказ `default`). Выполняется из `Headless/Source/main.cpp` (`pcc_ui_schema_self_test_failed`) и из UE-теста `GV2.Runtime.ContentCore.UiSchema`. Красный тест на откате подтверждён: отключение проверки циклов вызывает stack overflow / segfault, отключение проверки namespace даёт `pcc_ui_schema_self_test_failed: ui_schema.schema_ref.forbidden_namespace`.

- [x] **UPP-04 — Переносимый валидатор значения**
  - Зависимости: UPP-02.
  ...
  - Evidence: `Source/GV2ContentCore/Public/GV2ContentCore/UiSchema.h`, `Source/GV2ContentCore/Private/UiSchema.cpp` — функция `ValidateUiFieldValue` и вспомогательные функции `IsValidUiKey`, `ValidateTextSpec`, `ValidateBindingSpec`. Валидатор оперирует `GV2ContentCore::FValue` и `FDiagnostic`, отклоняет неизвестные ключи на всех уровнях вложенности (`core:diagnostic.ui_schema.value.unknown_field`), проверяет грамматику ключа, дубликаты в `keyed_by`-коллекциях, целевые виды `ref`/`TextSpec`/`BindingSpec`, границы массивов и применяет дефолтные значения. Conformance-тесты в `Source/GV2ContentCore/Private/UiSchemaConformance.cpp` (случаи 25–33) исполняются как в `gv2-headless --self-test`, так и в automation-тесте `GV2.Runtime.ContentCore.UiSchema`. Красный тест на откате подтверждён: снятие проверки замкнутости объекта даёт `pcc_ui_schema_self_test_failed: ui_value.object.unknown_field`, снятие проверки дубликатов ключей даёт `pcc_ui_schema_self_test_failed: ui_value.array.duplicate_key`.

- [x] **UPP-05 — Публикация UI-схем в репозитории и владение namespace**
