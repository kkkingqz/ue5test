...
- [x] **UPP-26 — Приведение контента и Lua**
  - Зависимости: UPP-25.
  - Done: в презентере и контенте не осталось формы, существующей ради старого транспорта; идентичность всех коллекций явная и семантическая — ни позиционной, ни выведенной из `resource_id`; свойства, которые раньше принимались и игнорировались, отсутствуют либо реализованы; форма каждого поля экрана локации проверяется против **скомпилированной** схемы, а не против записанного вручную списка ключей — тест краснеет при расхождении формы презентера и схемы; `gv2-headless --check-scripts` зелёный.
  - Evidence: `GameData/`, `Tests/Lua/presentation/`, `gv2-headless --check-scripts`.
  - **Пересмотр критерия (2026-08-25, по решению пользователя):** исходная формулировка требовала спеку конкретно **на стороне Lua**. Проверено: ни один другой пункт плана (ни задачи UPP-27…32, ни общий Definition of Done в `README.md`) от Lua-доступа к схеме не зависит — это было самостоятельное локальное требование этой одной задачи. Построить его означало бы добавить новую production-способность Lua-песочницы (biнding для чтения схемы) непропорционально масштабу «причесать контент/Lua» — по решению пользователя формулировка смягчена до «форма проверяется против скомпилированной схемы», без уточнения слоя, поскольку сама проверка ценна независимо от того, где она происходит.
  - **Статус (2026-08-25):** все четыре пункта проверены и выполнены. (1) В `GV2ScreenFieldAdapterRegistry` от location не осталось ни одного адаптера (`Adapters({})`), только generic-хелперы; в презентере (`GameData/textsystem/scripts/presentation/location_presenter.lua`) нет ни одной ветки ради старого транспорта. (2) Идентичность коллекций везде явная и семантическая: `items` (инвентарь) ключуется `item.instance_id`, `characters` — `char_entry.key` из контента экрана, `meters` — константным `"stamina"`, `commands`/travel-кнопки — `act.key`/`"travel_" .. path`; ни один ключ не выведен из `resource_id` и не позиционный (проверено чтением презентера и существующих `Tests/Lua/presentation/{declarative_location_spec,location_scene_boundary_spec}.lua`). (3) Ни одного игнорируемого свойства не осталось — `DescribeUiCapabilities` всех четырёх композитов (`GV2LocationCompositeWidgetBases.cpp`) объявляет ровно те поля, что есть в соответствующей `ui_field_location_*.v1` схеме, один в один, включая `portrait_resource_id` (через `AddImage`). (4) Форма каждого поля проверяется против реальной **скомпилированной** схемы, а не ручного списка ключей — закрыто UPP-27 (2026-08-25): `FGV2UiSchemaCache` (`Source/GV2/Private/UI/GV2UiSchemaCache.h/.cpp`) резолвит `schema_id`, сканируя реальные `*.schema.json5`, и `FGV2ScreenFieldAdapterRegistry::PrepareBindingDefinitions`/`BuildFields` валидируют форму каждого поля экрана локации против этой скомпилированной схемы на **каждом** реальном apply в production (не только в тесте, и для любой схемы, а не только для узкого `IsFlatBindableItemsSchema`-подмножества, как было раньше) — регрессия при расхождении формы презентера и схемы ловится тестами `GV2.Runtime.Presentation.RhStartOpensLocationScreen`/`GV2.Runtime.UI.LocationScreenTransitionContract`. `gv2-headless --check-scripts` зелёный (`{"ok":true,"status":"ok","modules_checked":41,...}`), вся связка `Tests/Lua/presentation/*.lua` проходит против настоящего `GameData` в `gv2-headless --self-test` (ctest `gv2_headless_self_test`, 68/68 портативных тестов зелёные). `closed_schema_spec.lua` (BAI-03) намеренно остаётся с ручным списком ключей как узкий регресс-тест конкретно закрытой схемы `location_commands.v1` — Lua-доступ к схеме по-прежнему не построен и не требуется этим пунктом после пересмотра критерия.
...
- [x] Форма презентера сверяется со скомпилированной схемой автоматически (в C++-рантайме на каждом apply, UPP-27).
