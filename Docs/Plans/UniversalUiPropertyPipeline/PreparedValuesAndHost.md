...
- [x] **UPP-11 — Harness наблюдаемости capability и гейт убывания legacy**
  - Зависимости: UPP-10.
  ...
  - Evidence: harness — `Source/GV2/Public/UI/GV2UiCapabilityObservability.h`, `Source/GV2/Private/UI/GV2UiCapabilityObservability.cpp` (`RunUiCapabilityObservabilityHarness`, `CaptureUiTargetState`); тест `GV2.UI.CapabilityObservabilityHarness` (`Source/GV2/Private/Tests/GV2UiCapabilityObservabilityTests.cpp`) — позитивный случай (Boolean/Number/Text, все реально наблюдаемы через реальный Prepare/Commit) и три негативных: (a) `int_gap`/`str_gap`/`key_gap` — реальные consumers `Integer`/`String`/`Key` сегодня не пишут в target (`not_distinguishable`, не синтетика — это существующий пробел, который и должен ловить harness), (b) `enabled_missing`/`percent_missing` на хосте без биндинга `WidgetTree` (`prepare_or_commit_failed`), (c) `icon_gap` (StableId без разрешаемого ресурса — `no_distinct_pair`) и `binding_gap` (реальный `IGV2UiBindingTarget` пока никем не реализован — `not_distinguishable`). Красный тест на откате подтверждён: временный no-op в `FGV2BooleanPropertyConsumer::Commit` даёт `Expected 'Boolean/Number/Text capabilities are observable' to be true` → красный; восстановление → зелёный.
  - Гейт убывания — `Tools/Content/validate_ui_pipeline_legacy_gate.py` (Python, по образцу `validate_core_decoupling.py`), зарегистрирован в `CMakeLists.txt` как `ui_pipeline_legacy_gate_contract` + `ui_pipeline_legacy_gate_negative_contract`. Текущие верхние границы, зафиксированные на 2026-08-24: 30 `Prepare`/`Build`-функций (все — в `Source/GV2/Private/Application/GV2ScreenFieldAdapterRegistry.cpp`), 13 payload-членов `FGV2ScreenFieldValue`, 20 schema-specific DTO в `GV2BridgeTypes.h` (29 структур минус 9 общих/envelope: `FGV2UiBindingHandle`, `FGV2UiControlValue`, `FGV2TextViewModel`, `FGV2ScreenFieldDescriptor`, `FGV2ScreenFieldValue`, `FGV2ScreenViewModel`, `FGV2ScreenInstanceViewModel`, `FGV2UiDocumentViewModel`, `FGV2SessionStatus`). `--self-test` доказывает обнаружение роста по каждой из трёх метрик на синтетических fixtures; красный тест на откате подтверждён вручную — добавление лишней `Prepare`-функции в реальный файл даёт `prepare_build_functions grew from 30 to 31`, откат восстанавливает зелёный.
  - Сборка `GV2Editor` и полный `GV2.UI.*` + `GV2.Runtime.ContentCore.UiSchema` (7 тестов) подтверждены зелёными через `UnrealEditor-Cmd`.

## Проверка milestone

- [x] Prepare не меняет физическое состояние ни на валидном, ни на невалидном входе.
- [x] Инъекция отказа Commit даёт описанное ADR наблюдаемое поведение.
- [x] Harness наблюдаемости краснеет во всех трёх сценариях подделки объявления.
- [x] Гейт убывания зафиксировал текущие числа и запрещает их рост.
- [x] Ни один виджет ещё не мигрирован.
