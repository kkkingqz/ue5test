...
- [ ] M1 — [Registry Integrity](RegistryIntegrity.md): заморозка становится необходимой, композиция — путём поиска, дубликат — ошибкой. EEH-01…05.
...
- [ ] Метод, полученный через `game.entity_extensions`, невозможно подменить после заморозки ни одним путём.
- [ ] Разрешение метода экземпляра и определения идёт через скомпонованную таблицу; `get_effective_methods` перестаёт быть неиспользуемым API.
- [ ] Повторное объявление метода внутри одного модуля отклоняется с той же ошибкой, что и между модулями.
- [ ] Повторная регистрация внутри `with_isolated_extensions` не переживает выход из изоляции.
