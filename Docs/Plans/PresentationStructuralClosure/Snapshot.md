...
- [x] **PSC-06 — Сделать snapshot владельцем и передать PrepareContext**
  ...
  - **Реализация (2026-09-08):** `FGV2PresentationPrepareContext` (`GV2SessionContentSnapshot.h`) — non-owning wrapper around `const FGV2SessionContentSnapshot&`, exposing `GetTheme()`, `ResolveScreen(...)`, `ResolveResource(...)`, `GetGameShellClass()`, `GetSchemaCache()`. Constructed only from an already-published/in-progress snapshot; никогда не global-accessible.
    ...
    **Theme/Text НЕ переведены на реального production caller** в этом изменении — только TYPE-level entry point (`GetTheme()`) существует. Найденный Prepare-phase вызов `GetConfiguredTheme()` находится внутри `UGV2TextPipeline::Resolve`, достижимого из `GV2ScreenFieldMaterializer::ResolveText`/`ProjectMaterializedValue`/`BuildFields`, и требует threading через несколько сигнатур. Recovery-related caller достигает `Resolve` до существования session snapshot; его текущий configured/null-context fallback является временным contract gap, а не допустимым target-исключением. `PSC-10B` обязан заменить этот путь на UE-native recovery с прямым `GetCoreMinimalTheme()` либо уже подготовленными minimal values и запретить configured accessor полностью. Остальные найденные call sites находятся в widget `NativePreConstruct`/`Apply`/`ResolveStyleClass` и относятся к Commit-phase resolution, то есть к исходной находке `PAH-R1`. Архитектурная починка требует resolved payload из `FGV2PreparedPresentationTransaction`: `PSC-10A` закрывает виды операций, `PSC-10B` — central style и recovery boundary. Это scope-граница `PSC-06`, а не заявление о допустимости fallback после завершения плана.
...
- [x] Snapshot является target owner; screen/resource Prepare использует explicit PrepareContext, а Theme/Text и удаление legacy accessors явно переданы `PSC-10A`/`PSC-10B`. (`PSC-06`, 2026-09-08 — Theme/Text entry point существует, но production caller ещё не переведён; это не считается выполненной частью M2)
