...
- [ ] **PSC-12 — Атомарно мигрировать Widget `UCLASS` paths и ассеты**
  ...
  - Done:
    ...
    - **точка возврата названа явно и проверена до начала**: ею является последний коммит перед началом миграции (на момент подготовки — `03271fe`, correction pass поверх `PSC-11`), дерево на нём целиком использует прежние пути; при обрыве миграции на любом шаге — включая частично пересохранённые ассеты и неснятые редиректы — восстановление выполняется возвратом рабочего дерева к этому коммиту целиком, а не доведением наполовину мигрированного состояния.
