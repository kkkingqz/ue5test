...
- [ ] `game.commands.handlers` доступен на фазе `register` и заморожен после неё.
- [ ] Каждый класс отказа регистрации покрыт negative case.
- [ ] Наблюдаемое поведение игры не изменилось: реестр пуст и никем не используется.
- [ ] `repository_content_hash` не изменился.
