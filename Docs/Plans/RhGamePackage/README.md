...
- [ ] Оба хоста и CLI собирают репозиторий из `GameData/core` и `GameData/rh`.
- [ ] Ни один `core:item.*`, `core:actor.*`, `core:location.*` не существует в `GameData/`.
- [ ] Поиск `rh:` по `Scripts/`, `GameData/core/` и `Source/` не даёт совпадений; это проверяется гейтом.
- [ ] Игра запускается, демо-экран отображается, переход между локациями работает.
- [ ] Сейв, снятый до переноса, не заявляется совместимым: отсутствующий `core:location.*` даёт типизированную ошибку, а не тихий сбой.
