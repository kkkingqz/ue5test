...
- [ ] Тексты и ресурсы перенесённых сущностей лежат в `rh` вместе с ними.
- [ ] PO-каталог `core` не содержит записей перенесённых текстов.
- [ ] Переход между локациями работает на `rh:location.*`.
