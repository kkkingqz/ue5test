...
updated: 2026-08-19
depends_on:
  - ../../../Proposals/Archive/RHActorsLuaSimplificationProposal.md
...
> **Материализует:** [RH Actors Lua Simplification Proposal](../../../Proposals/Archive/RHActorsLuaSimplificationProposal.md) и [ADR-0032](../../../ADR/0032-field-contracts-and-generic-instance-creation.md).
> **Задачи:** RAS-01…17.
...
- [x] `spend_gold` и `spend_stamina` не полагаются на контракт поля как на замену предусловию: проверка аргумента и явная диагностика `PreconditionNotChecked` сохранены осознанно.
...
- [x] Из `GameData/rh/scripts/gameplay/actors.lua` удалена таблица `RESOURCES`; проверка аргумента осталась одной локальной функцией согласно RAS-17.
...
## Уточнения по итогам приёмки

**План противоречил сам себе, и это моя ошибка при его доработке.** RAS-09 требовал удалить `validate_amount`, а добавленный позже RAS-17 требовал оставить одну локальную проверку аргумента. Обе задачи были отмечены выполненными, хотя выполнить их одновременно невозможно.

Реализация выбрала правильную сторону — позднее и более обдуманное решение: контракт поля защищает результирующее состояние и по построению не ловит `spend_gold(-10)`, поэтому проверка аргумента нужна. Неверными оказались формулировки RAS-09 и двух строк DoD; они исправлены выше.

Урок для последующих планов: добавление задачи в уже сформированный этап обязано сверяться с текстом соседних задач, а не только с общим замыслом.

**Проверка underflow в `spend_*` сохранена намеренно.** Предложение утверждало, что контракт поля делает её избыточной. Структурно это так: запись отрицательного значения отклоняется валидатором поля. Но сообщения различаются по смыслу — `PreconditionNotChecked` называет ошибку программиста («забыт `require_gold`»), а `FieldValidationConstraintFailed` называет симптом. Диагностика оказалась ценнее краткости.
