...
- [ ] Отказ валидатора оставляет state неизменным и не публикует фактов.
- [ ] Факты доставляются только после успешного commit.
- [ ] Порядок доставки и подписчиков воспроизводим и одинаков в обоих host-ах.
- [ ] Превышение лимита pump переводит session в `Failed`.
- [ ] Обработчик события не может изменить state напрямую.
- [ ] Сценарий перемещения проходит целиком в UE и headless.
