...
- [ ] **RAS-16 — Реестр видов экземпляров**
  - Зависимости: RAS-04.
  - Перечисление `"item"` в ядре нарушило бы [INV-016](../../Architecture/Invariants.md): `Item` и `Inventory` являются семантикой `rh` и не поднимаются ни в `textsystem`, ни в `core`.
  - Done: вид экземпляра объявляется пакетом на фазе `register` и замораживается вместе с остальными реестрами; имя секции `game.state` **выводится** из имени вида по общему правилу (`item` → `item_instances`), поэтому раскладка сейва не меняется и миграция не требуется; ядро не содержит ни одного имени вида; неизвестный вид — типизированный отказ; `actor` записан как осознанное исключение с делегированием в существующий реестр акторов.
  - Evidence: `Scripts/runtime/`, `Tests/Lua/actors/`.
...
- [ ] **RAS-06 — Нормализация дескрипторов и ссылок владельцев**
  ...
  - Evidence: `Scripts/authoring/context.lua`, `Scripts/runtime/`, `Tests/Lua/actors/`.
...
- [ ] В ядре нет ни одного имени вида экземпляра; неизвестный вид даёт типизированный отказ.
- [ ] Раскладка секций сейва не изменилась.
