...
- [ ] Пакет полностью заменяет `core:module.gameplay.root`, не вызывая базу.
- [ ] Пакет расширяет тот же модуль, делегируя базе необработанные команды.
- [ ] Два пакета на один `module_id` выстраиваются в детерминированную цепочку.
- [ ] Замещение запечатанного модуля и `core:module.*` без раннего провайдера отвергаются раздельно.
- [ ] Зависимости базы загружаются, даже если замещающий пакет их не объявил.
- [ ] Стек ошибки в модуле мода называет пакет-владельца.
