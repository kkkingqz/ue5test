...
- [ ] Хост собирает репозиторий из нескольких пакетов в явном порядке, повторно и детерминированно.
- [ ] Мод замещает definition ядра и Lua-модуль ядра; оба пути покрыты фикстурой, исполняемой обоими хостами.
- [ ] Замещающий модуль получает базу через `require_base()` и делегирует ей необработанные случаи.
- [ ] Замещение запечатанного модуля и создание нового `core:*` ID отвергаются раздельными типизированными диагностиками.
- [ ] Запись в таблицу экспорта после инициализации отвергается.
- [ ] Одинаковый набор пакетов даёт одинаковый `ScriptSetHash` и одинаковый run digest в UE и headless.
- [ ] Сейв фиксирует фактический набор пакетов и их fingerprints.
