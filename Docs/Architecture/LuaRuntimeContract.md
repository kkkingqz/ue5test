...
- **Отказ на дубликат внутри пакета**: повторная регистрация того же `command_id` внутри одного пакета без явного флага `override` отклоняется ошибкой `CommandHandlerDuplicateRegistration` и блокирует запуск сессии.
- **Явное замещение**: замена существующего обработчика другим пакетом разрешена только для команд с `replaceable = true`. Замещающий пакет обязан сохранять argument contract опубликованной команды.
...
- **Сохранение валидаторов**: валидаторы всех пакетов, нацеленные на команду, сохраняются и исполняются до вызова нового обработчика.
