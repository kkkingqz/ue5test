...
**Гейт привязки тестов к контенту (TSR-02, ADR-0046, план [TestSuiteRestructuring](../Plans/TestSuiteRestructuring/README.md)).** `Tools/Testing/validate_test_content_coupling.py` проверяет развязку тестового сьюта ядра от игрового контента (`test_content_coupling_contract`, `test_content_coupling_negative_contract`). Тесты разделены на две категории: contract-тесты механики (не вправе содержать привязок к игровому контенту и используют синтетические фикстуры `core:`) и content smoke (проверяют интеграцию на реальном контенте со структурными утверждениями, размещаясь только в файлах из `smoke_files`). Ratchet-гейт сверяет фактические привязки с `Tools/Testing/test_content_coupling_baseline.json`, запрещая появление любых новых привязок, рост baseline и превышение предварительного потолка размера тестового файла (3306 строк).
...
| Test content coupling & line limits | все C++ test sources и headers в `Source/**/Tests` | запрет новых обратных зависимостей на игровые пакеты (`rh`, `textsystem`, etc.) и контент-руты (`/Game/RH`, `/Game/TextSystem`); потолок размера тестового файла 3306 строк (кроме временного исключения `GV2RuntimeSubsystemTests.cpp`); потолок размера `RunTest` метода 480 строк | ratchet gate `Tools/Testing/validate_test_content_coupling.py` (CTest `test_content_coupling_contract`, `test_content_coupling_negative_contract`) на основе `test_content_coupling_baseline.json`; негативные тесты проверяют добавление новой привязки в contract-тест, искусственный рост baseline, превышение потолка строк файла и превышение потолка `RunTest` |
