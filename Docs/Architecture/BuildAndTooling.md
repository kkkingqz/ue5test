...
| Test fixture ownership | все source и header файлы в каталогах `Source/**/Tests` | вызовы `AddToRoot` и `RemoveFromRoot` разрешены исключительно внутри RAII-владельцев (`FScopedTestWorldContext`, `TScopedRootObject`); file-level allowlists запрещены; мутация `EGV2ForgeryMode` разрешена только через `FScopedForgeryMode` | scope-aware AST/block scan исходников тестов проверяет enclosing class/function; synthetic negative self-tests в `validate_test_fixture_ownership.py` (CTest `test_fixture_ownership_contract`, `test_fixture_ownership_negative_contract`) проверяют мутации внутри ранее разрешённых файлов; runtime isolation проверяется automation |
...
| `CFC-AF-03` (`REVIEW-03` / `STATUS-021`) | static gate `validate_session_snapshot_ownership.py` (запрет untraced owning raw/plain pointers на виджеты в prepared UI типах, 13 негативных мутаций в self-test), Game Thread guard в `FGV2PresentationApply::Apply` | `GV2.UI.PrepareCommitAndFailureInjection` (сценарии Prepare→GC→Commit, Reset→GC для коллекций и табов, откат сиблингов, отклонение off-thread Apply) |
...
Unreal runner обязан иметь UE 5.8 в `${UE_ROOT}` (default `/opt/unreal-engine`, переопределяется repository variable). Fork pull request не запускается на self-hosted runner. Прогон запускается в свежем процессе Editor (`UnrealEditor-Cmd`) со сбором полного discovery инвентаря `GV2` и машинного отчёта `-ReportExportPath`. Результат валидируется `Tools/Testing/ue_test_report.py` (`validate_run`): множество обнаруженных тестов обязано строго совпасть со всеми завершёнными записями, каждая запись обязана иметь статус `Success` и 0 ошибок, счётчики `total`/`passed`/`failed`/`skipped` согласованы (отсутствие обязательных счётчиков вызывает ошибку), а `run_identity` (`run_id`, `source_revision`, `source_diff_hash`, `build_fingerprint`) подтверждает соответствие актуальному коду и загруженным бинарникам без тавтологии. Исходная identity порождается в runtime C++ модулем `FGV2Module` (`GV2.Runtime.ModuleIdentity` и `runtime_identity.json`) через детерминированный SHA-256 хэш проектных `.so` файлов (`GV2ContentCore::FSha256Builder`) и сгенерированные при сборке ревизию и дифф (`GV2BuildIdentity.gen.h`), а runner независимо вычисляет эталонное значение (учитывая как `git diff HEAD`, так и неотслеживаемые файлы). Любые префиксы `missing_`, `unknown_`, `no_`, `error:`, расхождения, пропуски, состояние `NotRun`/`InProcess` или неидентифицированный отчёт являются безусловным отказом (fail-closed).
...
- Отчёт обязан содержать поддерживаемую версию схемы (`schema_version`);
...
MCP в уже запущенном Editor допускается как дополнительный developer run (`Tools/MCP/run_ue_tests.py`), но mismatch загруженных библиотек запрещает считать его freeze evidence. Во всём MCP polling и result flow (`GetTestStatus`, `GetTestResults`) строго требуется exact `task_id`. Generic «последний результат» (вызовы без `task_id` или с пустым телом `{}`) категорически запрещены как на уровне клиента (`mcp_client.py`), так и на уровне раннера (`run_ue_tests.py`). При асинхронном запуске раннер требует явный непустой `task_id` из ответа `RunTestsByFilter`; отсутствие `task_id` при статусе `in-process` или несовпадение возвращённого `task_id` со стороны сервера немедленно прерывает выполнение с ошибкой (fail-closed). Проверка неактуальности бинарников в запущенном процессе (`check_editor_binary_staleness`) опирается на инспекцию `/proc` в рамках текущего Linux baseline; на других платформах статус freeze evidence подтверждается только запуском `run_ue_acceptance.py --fresh-process`.
