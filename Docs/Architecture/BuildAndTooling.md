...
| Headless hash fields | declarations `FRunManifest`, `FRunDigest`, `FRunAcceptedCommand` и `FindField` calls в `GV2RunManifest.cpp`, `GV2RunDigest.cpp` | независимая таблица политик полей (required/optional, hash/non-hash, `IsCanonicalSha256`) | static regex scan declarations и call sites; synthetic negative self-tests в `validate_headless_hash_fields.py` (CTest `headless_hash_field_inventory_contract`, `headless_hash_field_inventory_negative_contract`); conformance проверяется `RunRunManifestConformance`, `RunRunDigestConformance` |
