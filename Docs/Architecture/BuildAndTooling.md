...
| Test content coupling & line limits | все C++ test sources и headers в `Source/**/Tests` | запрет новых обратных зависимостей на игровые пакеты (`rh`, `textsystem`, etc.) и контент-руты (`/Game/RH`, `/Game/TextSystem`); потолок размера тестового файла 3306 строк (кроме временного исключения `GV2RuntimeSubsystemTests.cpp`) | ratchet gate `Tools/Testing/validate_test_content_coupling.py` (CTest `test_content_coupling_contract`, `test_content_coupling_negative_contract`) на основе `test_content_coupling_baseline.json`; негативные тесты проверяют добавление новой привязки в contract-тест, искусственный рост baseline и превышение потолка строк |
