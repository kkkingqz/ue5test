...
| Session snapshot ownership | declarations и private members `FGV2SessionContentSnapshot`, `FGV2ResolvedScreenRegistry` и `UGV2ScreenRegistry` | snapshot и resolved registry не содержат указателей/ссылок на `UGV2ScreenRegistry`; `UGV2ScreenRegistry` не содержит мутируемого runtime-кэша (`ResolvedByScreenId`, `bBuilt`), метода `Build()` и члена `Resolve()`; `FGV2ResolvedScreenRegistry` удерживает классы виджетов через `TStrongObjectPtr<UClass>`; члены `FGV2SessionContentSnapshot` строго типизированы | static regex scan declarations; synthetic negative self-tests в `validate_session_snapshot_ownership.py` (CTest `session_snapshot_ownership_contract`, `session_snapshot_ownership_negative_contract`); runtime isolation проверяется automation `FGV2SessionScreenRegistrySnapshotIsolationTest` |
