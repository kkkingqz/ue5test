...
7. ~~**Политика отказа зависит от владельца схемы.**~~ **Отозвано (`DCA-20`, 2026-09-06)** — см. «Implementation caveat» ниже. Исходная формулировка: несовместимая схема `core`/`textsystem`/`rh` — ошибка сборки проекта: сессия не становится `Ready`. Несовместимая схема мода отклоняет сам мод с типизированной диагностикой; сессия продолжается без него. Сторонние данные не должны иметь возможности сделать игру незапускаемой.
...
## Resolution: UI schema authority (`GBH-03`/`REM-04`, historical)

Decision 6 (владение schema ID по namespace) и Decision 7 (политика отказа зависит от владельца схемы) изначально применялись к `ui_field`/`ui_value` схемам не полностью: `FGV2UiSchemaCache` резолвила их статичным сканированием фиксированного списка файловых корней (`GameData/core`, `GameData/textsystem`, `GameData/rh`, `GameData/sample`, включая `sample` — пакет, отсутствующий в `GameData/mods.lock.json5`), регистрируя каждую схему по её собственному полю `id` без проверки, что его namespace совпадает с пакетом-источником, и без связи с pinned `GameDataRepository`/package closure текущей сессии.

**`DCA-19` (2026-09-06) закрыл часть про Decision 6**: корни схем теперь читаются из пиннингованного замыкания (`GameData/mods.lock.json5` через `GV2PackageClosure::DiscoverFromGameData`, общее с `UGV2RuntimeSubsystem` и `UGV2ScreenRegistry`), `sample` больше не сканируется, а схема с namespace `id`, не совпадающим с пакетом, под которым она физически найдена, не регистрируется. Decision 6 действует без изъятия.

**`DCA-20` (2026-09-06) отозвал Decision 7**, а не реализовал её: `IsModNamespace`/`Diag.bFatal` (`GV2UiCapability.cpp`) вычисляли владельца схемы закрытой тройкой литералов, дословно копирующей `mods.lock.json5`, но результат не читал ни один production-путь — все три вызывающих ветвились по общему `bSuccess`, одинаковому для мода и первопартийной схемы, и `bFatal` проверяли только два самодостаточных теста, ничего не наблюдавших в поведении сессии. В проекте нет ни одного мода, поставляющего `ui_field`/`ui_value` схему, и нет механизма «отклонить мод, продолжить сессию без него» ни для одного другого типа контента — реализация Decision 7 потребовала бы такого механизма с нуля, ради консьюмера, которого не существует (`ADR-0026`). Мёртвая поверхность физически удалена; при появлении первого реального мода со собственной `ui_field`/`ui_value` схемой Decision 7 может быть переформулирована и принята заново вместе с консьюмером, который её проверит.
