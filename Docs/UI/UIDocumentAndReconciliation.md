...
> **Проверки:** `GV2.UI.LayeredReconciliation.*`, `GV2.Runtime.Session.PreparedCommitAndFailureInjection`, `GV2.Runtime.Presentation.*`, `GV2.Runtime.UI.GameShellViewportFill`, `gv2-headless --self-test`.
...
| --- | --- | --- | --- |
| `CommitUiHostProperties` | `PropertyMutation` | `RollbackCommittedMutations` replay previous plan | `GV2.UI.Consumers.BasicWidgetsAndHosts`, `GV2.UI.Consumers.KeyedCollectionAndRollback` |
| `UGV2ScreenWidgetBase::CommitScreenFields` | `ScreenFields` | `RollbackFieldPlans` | `GV2.UI.LayeredReconciliation.ReusedScreenCommitRollback` |
| `FGV2LayeredUiReconciler::CommitReconcile` | `Document` | `RollbackFieldPlans` for committed screens | `GV2.UI.LayeredReconciliation.MultiLayerCommitFailure` |
| `FGV2KeyedCollectionPropertyConsumer::CommitWithFailureInjector` | `KeyedCollection` | inverse item plan through `CommitUiHostProperties` | `GV2.UI.Consumers.KeyedCollectionAndRollback` |
| `FGV2TabContainerTabsPropertyConsumer::CommitWithFailureInjector` | `NestedScreenTabs` | `RollbackFieldPlans` for committed child screens | `GV2.Runtime.UI.NestedScreenReconciliationContract` |
| `UGV2GameShellWidgetBase::AttachScreenToLayer` | `ShellAttach` | atomic single-widget `AddChild`; no compensating rollback of its own to run -- `CommitReconcile` (PAH-06B) no longer calls it for document reconciliation, which goes through `FGV2KeyedCollection::ReconcilePrepared` (`Document` boundary above) instead; remains a general single-widget attach primitive, still directly exercised outside the reconcile path | `GV2.UI.LayeredReconciliation.BasicLifecycle` |
...
Общий animated swap документных экранов пока не реализован и не является частью baseline: такие участники по-прежнему снимаются синхронно. Реальный промежуток между логическим и физическим снятием реализован для host-local hover-окна: stale-удаление немедленно гасит ввод и доигрывает fade, self-dismissal остаётся интерактивным и отменяемым. Поэтому правило двух видов ухода проверяется на production-потребителе, а не выполняется пусто.
...
Automation-тесты (`GV2.UI.LayeredReconciliation.*`, `GV2.Runtime.Presentation.*`, `GV2.Runtime.UI.GameShellViewportFill`, `gv2-headless --self-test`) проверяют:
