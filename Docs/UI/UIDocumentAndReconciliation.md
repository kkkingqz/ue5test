...
1. **Контейнерная атомарность структуры**: `FGV2KeyedCollection` гарантирует атомарность иерархии детей (`UPanelWidget`). Добавление, удаление и переупорядочивание детей фиксируются в контейнере только после успешного завершения валидации, создания всех виджетов и вызовов `ApplyItem`.
2. **Фаза preflight-валидации**: проверка обязательности ключей, их уникальности и вызов `CanApplyItem` происходят до создания новых виджетов и до вызовов `ApplyItem` на переиспользуемых виджетах. Ошибка на этой фазе не мутирует ни контейнер, ни живые виджеты.
3. **Отказ внутри `ApplyItem`**: `FGV2KeyedCollection` оперирует обобщёнными виджетами (`WidgetType`) и не хранит снимков их внутреннего состояния. При отказе единичного `ApplyItem` структура контейнера остаётся нетронутой, а полный откат состояния полей экрана обеспечивается внешним уровнем реконсилятора экрана (`UGV2ScreenWidgetBase::ApplyScreenFields` через механизм `CaptureScreenField` / `ApplyScreenField`).
