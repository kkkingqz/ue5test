...
1. **Поверхность холодного восстановления** — нативный экран `UGV2RecoveryScreenWidget` и код `UGV2RuntimeSubsystem`, который строит его строки при провале bootstrap. Это две единицы кода, а не один класс, и роль определяется не именем класса, а условием: обе выполняются именно тогда, когда у сессии нет snapshot вовсе (bootstrap ещё не дошёл до `Ready` либо сессия уже `Failed`). Обе берут из минимальной темы базовую типографику и аварийные строки каталога (`core:text.screen.recovery.title`, `core:text.screen.error.*`). Это bootstrap/failure-time fallback без snapshot, а не повторное разрешение snapshot-owned Theme.
2. **Построение самого snapshot** — candidate builder один раз резолвит минимальную тему и копирует её effective text catalog в value-owned `FGV2ResolvedUiTheme::CoreMinimalFallbackTextCatalog`: `text_id`, которого нет ни в `TextCatalog`, ни в `FallbackTextCatalog` настроенной темы, разрешается из этой копии. Каталог входит в canonical Theme value и поэтому меняет `presentation_hash`/`session_content_id`. Это не второй authority и не runtime pull — значение разрешается там же, где и остальные поля snapshot, и downstream видит его исключительно как обычное prepared value через `FGV2PresentationPrepareContext`; ни один Commit-facing путь к `GetCoreMinimalTheme()` не обращается.
