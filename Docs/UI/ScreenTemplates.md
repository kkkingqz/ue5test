...
1. `PrepareBindingDefinitions` принимает обязательный `const FGV2PresentationPrepareContext& PrepareContext`, выполняет детерминированный обход скомпилированной UI-схемы поля (`GV2ContentCore::FCompiledUiFieldSpec`), полученной из `PrepareContext.GetSchemaCache()`, и значений из Lua, собирая определения биндингов `FGV2UiBindingDefinition`.
2. После подготовки candidate binding set в `FGV2UiBindingRegistry` функция `BuildFields` (также требующая `const FGV2PresentationPrepareContext& PrepareContext`) потребляет выданные `FGV2UiBindingHandle` и материализует candidate values в типизированные структуры `FGV2ScreenFieldValue`.

Каждый `FGV2ScreenFieldValue` содержит `FieldId` (`FName`), `SchemaId` (`FString`), материализованное значение `PreparedValue` (`TSharedPtr<const FGV2PreparedUiObject>`) и скомпилированную схему `CompiledSchema` (`std::shared_ptr<const FCompiledUiFieldSpec>`). C++ schema-specific классы-адаптеры отсутствуют. Процесс-глобальный кеш схем (`GSessionSchemaCache`) и методы его ручной перестройки/очистки (`RebuildSchemaCacheForSession`, `ReleaseSchemaCacheForSession`) полностью удалены (CFC-04): единственным источником UI-схем является immutable snapshot сессии, доступный через `FGV2PresentationPrepareContext`.
