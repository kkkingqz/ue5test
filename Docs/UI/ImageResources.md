...
Каждый визуальный примитив обязан явно объявить политику масштабирования (`EGV2PrimitiveScalePolicy`). Значением по умолчанию является `Unset`, при котором применение изображения отклоняется. Политика проверяется на совместимость с объявленным `EGV2ImageRenderMode` ресурса на этапе Prepare до мутации виджета:

| Политика примитива | Совместимый режим ресурса | Поведение |
|---|---|---|
| `Unset` | — | Не задана (значение по умолчанию); применение ресурса отклоняется с диагностикой `unset_scale_policy` |
