...
**Package** — один каталог, чьё имя равно `package_id`. Внутри `definitions/` и `schemas/`. Базовый набор поставки состоит из пакетов: `GameData/core` (движок: framework-схемы, базовые экраны, тексты, ресурсы) и `GameData/rh` (игра: схемы предметов/локаций/расширений акторов, конкретные предметы, локации, персонажи, ресурсы и тексты). Движок не знает сущностей и предметных схем игры, а игра объявляет собственные схемы и строится по ним.

**Definition** — запись в `definitions/*.json5`. Имеет Stable ID, тип и поле `data`, устройство которого задаёт схема.

**Schema** — самоописывающийся файл в `schemas/`: объявляет `definition_type`, версию и допустимые поля с их типами и ограничениями. Схемы читает и рантайм, и инструменты — поэтому `gv2-content describe GameData/rh <тип>` показывает поля, не заглядывая в документацию. Framework-схемы объявляет `core`, а предметные схемы игры объявляет пакет `rh`.
