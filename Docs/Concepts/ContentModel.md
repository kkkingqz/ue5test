...
**Package** — один каталог, чьё имя равно `package_id`. Внутри `definitions/` и `schemas/`; всё остальное discovery не сканирует. Игра сегодня использует ровно один пакет — `GameData/core`.
