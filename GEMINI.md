...
- Перед тем как поставить отметку `[x]` в любом файле плана (`Docs/Plans/`), Gemini обязан:
...
- Если специализированный инструмент (например, `unreal-mcp`) недоступен, тест падает или хотя бы одно требование не может быть выполнено физически:
  ...
  - Gemini обязан немедленно остановиться, не ставить ложную отметку о готовности и дать пользователю честное сообщение о возникшей проблеме с точным описанием того, что заблокировано и что осталось сделать.
