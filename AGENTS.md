...
6. **Проверка стоит на пути, которым ходит продукт.** Инвариант не считается проверенным на основании юнит-теста его вспомогательной функции: нужен хотя бы один тест через production-путь. Правило добавлено по подтверждённому случаю — `UGV2ScreenRegistry::IsLayerAllowedForEmbedded` имела три зелёных утверждения в тесте при нуле вызовов из production-кода, то есть проверка размещения экрана была мёртвой поверхностью, а её тест это скрывал. Три предыдущих пункта касаются того, **что** перебирается; этот — **где** это исполняется: множество может быть перечислено верно, а перечислитель стоять в стороне от пути, который он охраняет ([ADR-0042](Docs/ADR/0042-presentation-authority-and-publication.md), `INV-P6` исходного ревью).
...
- проверка инварианта, покрытая только вызовом вспомогательной функции из теста, при отсутствии её вызова на production-пути;
...
**Когда change set меняет и проверку, и проверяемый артефакт.** Если change set добавляет или ужесточает проверку и тем же change set меняет артефакт, который эта проверка проверяет, сообщение обязано ответить, чем артефакт был неправ **до** появления проверки — а не почему он теперь проходит новую проверку. Ответ вида «чтобы проверка проходила» в любой формулировке («под гейт», «для соответствия тесту») не засчитывается: он описывает проверку, а не продукт, и направление обоснования при этом теряется — совместное изменение перестаёт быть отличимым от подгонки продукта под тест задним числом. Это не запрет менять артефакт и проверку одним change set — это обычная и правильная практика; правило касается только направления обоснования в сообщении. Разобранный прецедент: `WrapSize=1200` в `WBP_CommandPanel` появился тем же коммитом (`c2526ae`), что и новая строгая geometry-проверка, с причиной «предотвращение переполнения по вертикали» — состоянием, которого до этой проверки не наблюдал никто (см. [GV2_CommandPanelWrapSizeFixedNonScaling_2026-09-03.md](Docs/Status/GV2_CommandPanelWrapSizeFixedNonScaling_2026-09-03.md)).
